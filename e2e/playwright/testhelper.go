...
func NewTestHelper(t require.TestingT, name string) (*TestHelper, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("无法调用Playwright: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("无法启动浏览器: %v", err)
	}

	context, err := browser.NewContext()
	if err != nil {
		return nil, fmt.Errorf("无法创建上下文: %v", err)
	}

	page, err := context.NewPage()
	if err != nil {
		return nil, fmt.Errorf("无法创建页面: %v", err)
	}
...
func (pr *PlaywrightRequire) True(t *testing.T, value bool, msgAndArgs ...interface{}) {
	pr.Assert(t, func() error {
		var err error
		if !value {
			err = fmt.Errorf("预期值为真，但测试结果为假 '%s'", t.Name())
		}
		return err
	}, msgAndArgs...)
...
func (pr *PlaywrightRequire) False(t *testing.T, value bool, msgAndArgs ...interface{}) {
	pr.Assert(t, func() error {
		var err error
		if value {
			err = fmt.Errorf("预期值为假，但测试结果为真 '%s'", t.Name())
		}
		return err
	}, msgAndArgs...)
...
func (pr *PlaywrightRequire) NoError(t *testing.T, err error, msgAndArgs ...interface{}) {
	pr.Assert(t, func() error {
		var assertErr error
		if err != nil {
			assertErr = fmt.Errorf("预期不会出错，但测试中却出现了错误 '%s': %v", t.Name(), err)
		}
		return assertErr
	}, msgAndArgs...)
...
func (pr *PlaywrightRequire) Error(t *testing.T, err error, msgAndArgs ...interface{}) {
	pr.Assert(t, func() error {
		var assertErr error
		if err == nil {
			assertErr = fmt.Errorf("测试中预期会报错，但实际并未报错。 '%s'", t.Name())
		}
		return assertErr
	}, msgAndArgs...)
...
func (pr *PlaywrightRequire) Contains(t *testing.T, text, expected string, msgAndArgs ...interface{}) {
	pr.Assert(t, func() error {
		if !strings.Contains(text, expected) {
			return fmt.Errorf("预期文本包含 '%s' 但实际得到 '%s'", 预期, 文本)
		}
		return nil
	}, msgAndArgs...)
}
