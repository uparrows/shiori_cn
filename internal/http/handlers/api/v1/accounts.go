...
func HandleListAccounts(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		deps.Logger().WithError(err).Error("获取帐户时出错")
...
func HandleCreateAccount(deps model.Dependencies, c model.WebContext) {
	...
	if errors.Is(err, model.ErrAlreadyExists) {
		response.SendError(c, http.StatusConflict, "账户已存在")
		return
	}

	if err != nil {
		deps.Logger().WithError(err).Error("创建账户时出错")
		response.SendInternalServerError(c)
		return
	}
...
func HandleDeleteAccount(deps model.Dependencies, c model.WebContext) {
	...
	if errors.Is(err, model.ErrNotFound) {
		response.SendError(c, http.StatusNotFound, "未找到帐户")
		return
	}

	if err != nil {
		deps.Logger().WithError(err).Error("删除帐户时出错")
		response.SendInternalServerError(c)
		return
	}
...
func HandleUpdateAccount(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		deps.Logger().WithError(err).Error("更新帐户时出错")
		response.SendInternalServerError(c)
		return
	}
