...
func (p *updateCachePayload) IsValid() error {
	...
	for _, id := range p.Ids {
		if id <= 0 {
			return fmt.Errorf("id 值不能为 0 或负数")
		}
	}
	return nil
}
...
func HandleBookmarkReadable(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		response.SendError(c, http.StatusBadRequest, "无效的书签 ID")
		return
	}

	bookmark, err := deps.Domains().Bookmarks().GetBookmark(c.Request().Context(), model.DBID(bookmarkID))
	if err != nil {
		response.SendError(c, http.StatusNotFound, "未找到书签")
		return
	}
...
func HandleUpdateCache(deps model.Dependencies, c model.WebContext) {
	...
	if err := json.NewDecoder(c.Request().Body).Decode(&payload); err != nil {
		response.SendError(c, http.StatusBadRequest, "无效的请求负载")
		return
	}
	...
	if err != nil {
		response.SendError(c, http.StatusInternalServerError, "获取书签失败")
		return
	}
	...
	for i, book := range bookmarks {
		...
		go func(i int, book model.BookmarkDTO) {
			...
			if err != nil {
				deps.Logger().WithError(err).Error("更新书签缓存时出错")
				chProblem <- book.ID
				return
			}
...
func (p *bulkUpdateBookmarkTagsPayload) IsValid() error {
	if len(p.BookmarkIDs) == 0 {
		return fmt.Errorf("bookmark_ids 不应为空")
	}
	if len(p.TagIDs) == 0 {
		return fmt.Errorf("tag_ids 不应为空")
	}
	return nil
}
...
func HandleGetBookmarkTags(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		response.SendError(c, http.StatusInternalServerError, "检查书签是否存在失败")
		return
	}
	if !exists {
		response.SendError(c, http.StatusNotFound, "未找到书签")
	...
	if err != nil {
		response.SendError(c, http.StatusInternalServerError, "获取书签标签失败")
		return
	}
...
func (p *bookmarkTagPayload) IsValid() error {
	if p.TagID <= 0 {
		return fmt.Errorf("tag_id 应为正整数")
	}
	return nil
}
...
func HandleAddTagToBookmark(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		...
		if errors.Is(err, model.ErrTagNotFound) {
			response.SendError(c, http.StatusNotFound, "标签未找到")
			return
		}
		response.SendError(c, http.StatusInternalServerError, "无法向书签添加标签")
		return
	}
...
func HandleRemoveTagFromBookmark(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		...
		response.SendError(c, http.StatusInternalServerError, "从书签中移除标签失败")
		return
	}
...
func HandleBulkUpdateBookmarkTags(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		...
		response.SendError(c, http.StatusInternalServerError, "书签更新失败")
		return
	}
