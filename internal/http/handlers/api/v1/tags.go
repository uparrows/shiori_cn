...
func HandleListTags(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		deps.Logger().WithError(err).Error("获取标签失败")
...
func HandleGetTag(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		response.SendError(c, http.StatusBadRequest, "无效的标签 ID")
		return
	}
...
func HandleCreateTag(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		response.SendError(c, http.StatusBadRequest, "无效的请求体")
		return
	}

	if tag.Name == "" {
		response.SendError(c, http.StatusBadRequest, "标签名称为必填项")
		return
	}

	createdTag, err := deps.Domains().Tags().CreateTag(c.Request().Context(), tag)
	if err != nil {
		deps.Logger().WithError(err).Error("创建标签失败")
		response.SendInternalServerError(c)
		return
	}
...
func HandleUpdateTag(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		...
		deps.Logger().WithError(err).Error("标签更新失败")
		response.SendInternalServerError(c)
		return
	}
...
func HandleDeleteTag(deps model.Dependencies, c model.WebContext) {
	...
	if err != nil {
		...
		deps.Logger().WithError(err).Error("删除标签失败")
		response.SendInternalServerError(c)
		return
	}
