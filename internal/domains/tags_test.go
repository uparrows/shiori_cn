...
func TestTagsDomain(t *testing.T) {
	...
	t.Run("ListTags_WithBookmarkCount", func(t *testing.T) {
		...
		require.NotZero(t, foundTag.ID, "应该找到测试标签")
		assert.Equal(t, int64(1), foundTag.BookmarkCount, "标签的收藏数应为 1")
	})
	...
	t.Run("ListTags_WithBookmarkID", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, tags, 1, "应该返回书签的唯一标签")
		assert.Equal(t, testTags[0].Name, tags[0].Name, "应该返回书签的正确标签")
		...
		require.NoError(t, err)
		require.Len(t, tags, 1, "应该返回书签的唯一标签")
		assert.Equal(t, testTags[1].Name, tags[0].Name, "应该返回书签的正确标签")
	})
	...
	t.Run("ListTags_WithBothOptions", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, tags, 1, "应该返回一个标签")
		assert.Equal(t, tag.Name, tags[0].Name, "应返回正确的标签")
		assert.Equal(t, int64(1), tags[0].BookmarkCount, "标签的收藏数应为 1")
	})
	...
	t.Run("CreateTag", func(t *testing.T) {
		...
		assert.Greater(t, createdTag.ID, 0, "创建的标签应具有有效的 ID")
		...
		for _, tag := range allTags {
			if tag.Name == "new-tag" {
				found = true
				assert.Greater(t, tag.ID, 0, "数据库中的标签应具有有效的 ID")
				break
			}
		}
		assert.True(t, found, "数据库中应该能找到创建的标签。")
	})
